...
class SPALPA(nn.Module):
    ...
    def __init__(self,
                 ...
                 conv_args=None,
                 sampler='fps',
                 feature_type='dp_fj', # 输入特征拼接类型
                 use_res=False, # 是否残差
                 is_head=False, # 是否head
                 gamma=16,
                 num_gp=16, # 锚点数量
        ...
        # 是head就不聚合，否则聚合 既不是头又不聚合就可以残差链接
    ...
    # updated_local_f 是LPA updated_global_f是SPA
    def forward(self, pf):
        # pf分别是原始点坐标与特征
        p, f = pf
        # 第一个模块 就只运算 然后返回
