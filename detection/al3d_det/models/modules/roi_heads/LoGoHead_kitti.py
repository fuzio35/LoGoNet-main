...
class RoIHeadTemplate(nn.Module):
    ...
    def assign_targets(self, batch_dict):
        ...
        roi_center = rois[:, :, 0:3]
        # 沿着ROI的角度进行转换
        roi_ry = rois[:, :, 6] % (2 * np.pi) # 保证角度在范围内
        gt_of_rois[:, :, 0:3] = gt_of_rois[:, :, 0:3] - roi_center # GT要相对ROI进行平移
        gt_of_rois[:, :, 6] = gt_of_rois[:, :, 6] - roi_ry

        # transfer LiDAR coords to local coords
        #坐标系变换 从全局坐标系转到ROI局部坐标系
    ...
    def get_box_reg_layer_loss(self, forward_ret_dict):
        ...
        code_size = self.box_coder.code_size # 框编码大小
        reg_valid_mask = forward_ret_dict['reg_valid_mask'].view(-1) # 有效回归掩码--在计算阈值的时候确定
        # 源数据可能包含其他信息
        gt_boxes3d_ct = forward_ret_dict['gt_of_rois'][..., 0:code_size] # 拿到GT
        gt_of_rois_src = forward_ret_dict['gt_of_rois_src'][..., 0:code_size].view(-1, code_size) # 拿到元数据
        rcnn_reg = forward_ret_dict['rcnn_reg']  # (rcnn_batch_size, C) 回归输出
        roi_boxes3d = forward_ret_dict['rois'] # ROI部分
...
class VoxelAggregationHead(RoIHeadTemplate):
    ...
    # ROI网格池化 转化为对应的大小
    # ROI是基于锚框生成的-- 从ROI区域进一步提取特征
    # 每一个ROI包含若干网格；对这些网格进行特征提取与聚合
    # 全局ROI坐标是整个坐标系的位置
    # 局部ROI坐标是相对ROI中心的位置
    ...
    def get_dense_grid_points(rois, batch_size_rcnn, grid_size):
        ...
        # 获取非0索引  6*6*6,3 是所有非0点的位置
        dense_idx = faked_features.nonzero()  # (N, 3) [x_idx, y_idx, z_idx] N = 216 返回的是每个网格的索引
        # 重复 B * N * 3  
        ...
        # 获取相对于 ROI 的网格点坐标
        # 前半部分坐标归一化 -- 乘以局部尺寸，得到网格点在ROI坐标系的位置相对位置
    ...
    def forward(self, batch_dict):
        ...
        if self.pool_cfg.get('DENSITYQUERY', {}).get('ENABLED'):
            ...
            # B * N * G ,4 表示带batchid的3D网格全局坐标；就是每个ROI的网格编号都带上了batch
